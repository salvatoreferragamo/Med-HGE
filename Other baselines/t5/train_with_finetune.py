...
# from joint_clf import jointClf
...
def train_model(model, adam, train_data, dev_data, tokenizer, device, args):
    ...
    for epoch in range(args.num_epoch):
        ...
        for i, cur in enumerate(tqdm(train_data, desc='Epoch {}:'.format(epoch))):
            ...
            # prob = model(**cur)[0]
            # prob, encoder_hidden_states = model(**cur)[0], model(**cur)[-1][0]
            # flops, params = profile(model,(*cur,))
            # print('flops: ', flops, 'params: ', params)
            prob = model(**cur,output_attentions=True).logits
            # prob = model(**cur).logits
...
def init_argument():
    ...
    parser.add_argument('--pretrain_model', default='./t5_pegasus_pretrain/chinese_t5_pegasus_base')
    parser.add_argument('--model_dir', default='./saved_model')

    parser.add_argument('--num_epoch', default=10, help='number of epoch')
...
if __name__ == '__main__':
    ...
    # Joint Model 
    # joint_model = jointClf(args).to(device)
    
    if args.data_parallel and torch.cuda.is_available():
        device_ids = range(torch.cuda.device_count())
        model = torch.nn.DataParallel(model, device_ids=device_ids)
        # joint_model = torch.nn.DataParallel(joint_model, device_ids=device_ids)
