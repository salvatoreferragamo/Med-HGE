...
#         # source, target = ' '.join(source), ' '.join(targets)
#             # print(target)
#         scores = Rouge().get_scores(hyps=source, refs=target)
#         r1 += scores[0]['rouge-1']['f']
#         r2 += scores[0]['rouge-2']['f']
...
#         'rouge-l': 0.0,
#     }
#     for source, target in tzip(sources, targets):
#         # print(source, target)
...
#     parser.add_argument('--pred_path', type=str, default='../../dataset/test.json', help='pred file path')
...
#     #         # print(''.join(line.replace('<q>','\t').strip().split()))
#     #         golds.append(''.join(line.replace('<q>','\t').strip().split()))
...
#                 # print(''.join(line.replace('<q>','\t').strip().split()))
#                 # preds.append(''.join(line.replace('<q>','\t').strip().split()))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument('--gold_path', type=str, default='../dataset/dev.json', help='gold file path')
    parser.add_argument('--pred_path', type=str, default='./data/predict_result.tsv', help='pred file path')
    parser.add_argument('--dataset', default='test', type=str)
    parser.add_argument('--target', default='t5', type=str)
